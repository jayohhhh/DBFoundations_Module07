...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_JOu')
	 Begin 
	  Alter Database [Assignment07DB_JOu] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_JOu;
	 End
	Create Database Assignment07DB_JOu;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_JOu;
...
Select * from vProductInventoriesWithPreviousMonthsCounts
go
...
AS
	Select 
		...
		--CountPrevious = IIF(Month(InventoryDate) = 'January' ,0, Lag(sum(Count)) Over(Order by ProductName, InventoryDate))
		[CountPrevious],
		[CountPreviousKPI] = Case
			When Count > CountPrevious Then 1
			When Count = CountPrevious Then 0
			When Count < CountPrevious Then -1
			End
	From vProductInventoriesWithPreviousMonthsCounts
Go
...
-- <Put Your Code Here> --
Create Function dbo.fProductInventoriesWithPreviousMonthCountsWithKPIs(@CountPreviousKPI int)
	Returns Table 
	AS
		Return(
			Select
			...
			Where [CountPreviousKPI] = @CountPreviousKPI
		)
go
